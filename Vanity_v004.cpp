...
void VanitySearch::getCPUStartingKey(int thId, Int& key, Point& startP) {
  ...
  //
  int lrand = 2;//lrand = 1;
  key2.ShiftL((uint32_t)(Random_bits - lrand));
  printf("[i] Random Keys - Low Keys Space 2^(%d-%d) 0x%s \n", Random_bits, lrand, key2.GetBase16().c_str());
  //
  ...
  if (rekey > 0) {
	...
	if (Key_bits_length < (Random_bits - lrand)) goto NewRandom;// check
...
void VanitySearch::getGPUStartingKeys(int thId, int groupSize, int nbThread, Int *keys, Point *p) {
  ...
  int lrand = 2;//lrand = 1;
  key2.ShiftL((uint32_t)(Random_bits - lrand));
  printf("[i] GPU Random Keys - Low Keys Space 2^(%d-%d) 0x%s \n", Random_bits, lrand, key2.GetBase16().c_str());
  //
  //key2.ShiftL((uint32_t)(Random_bits - 1));
  key3.ShiftL((uint32_t)Random_bits);
  key3.Sub(&one1);// for strcmp()

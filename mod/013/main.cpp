...
void printUsage() {
  ...
  printf(" -start startKey: Set Starting key, default is random bits %d\n", (int)TARGET_KEY_BITS);
  printf(" -bits number: Set Random bits, default is random bits %d\n", (int)TARGET_KEY_BITS);
  printf(" -level number: Set number 1 to Enable PBKDF2 Seed function, default: -level 0\n");
  printf(" -bip39 number: Enable Generate Seed of BIP-39 word list use PRNG. Set number 1-%d \n", (int)BIP39_MAX_WORD); 
  printf(" -brainwallet number: Enable Brainwallet algoritm. Set number 1-%d \n", (int)BIP39_MAX_WORD); 
  ...
  printf(" -verbose number: Set 0-4 Verbose level info, default 1\n");
  exit(0);
...
int main(int argc, char* argv[]) {
  ...
  bool stop = false;
  bool bip39 = false;
  bool brainwallet = false;
  int nb_word = 12;// seed of bip-39 word list
  int searchMode = SEARCH_COMPRESSED;
  vector<int> gpuId = {0};
  vector<int> gridSize;
  string seed = "";
  string start_key = "";
  int rand_bit = (int)TARGET_KEY_BITS;// Random Bits
  int FuncLevel = 0;// Enable functions case 1-4
  ...
  while (a < argc) {
    ...
    if (strcmp(argv[a], "-gpu")==0) {
      ...
      stop = true;
      a++;
    } else if (strcmp(argv[a], "-bip39") == 0) {
      bip39 = true;
      a++;
	  nb_word = atoi(argv[a]);
	  a++;
	}
	else if (strcmp(argv[a], "-brainwallet") == 0) {
		brainwallet = true;
		a++;
		nb_word = atoi(argv[a]);
		a++;
  ...
  // BIP-39 / Brainwallet
  if (nb_word > (int)BIP39_MAX_WORD) nb_word = (int)BIP39_MAX_WORD;
  if (nb_word < 1) nb_word = 1;
  if (brainwallet) bip39 = true;
  ...
  printf("[                                                           ]\n");
  printf("[===========================================================]\n");
  printf("[                Changes by Alek76 modify %s         ]\n", RELEASE_mod);
  printf("[                Match P2PKH address in GPU!!!              ]\n");
  printf("[===========================================================]\n");
  printf("[          Tips: 1NULY7DhzuNvSDtPkFzNo6oRTZQWBqXNE9         ]\n");
  printf("[===========================================================]\n");
  //printf("[    Options added argv [-start] [-bits] and disable SSE    ]\n");
  printf("[    Options added argv [-start] [-bits] and enable SSE     ]\n");
  printf("[===========================================================]\n");
  printf("[    Options added argv [-bip39] [-level] [-brainwallet]    ]\n");
  ...
  VanitySearch *v = new VanitySearch(secp, prefix, seed, start_key, rand_bit, FuncLevel, searchMode, gpuEnable, stop, bip39, brainwallet, nb_word, flag_verbose, outputFile, sse,

...
VanitySearch::VanitySearch(Secp256K1 *secp, vector<std::string> &inputPrefixes, string seed, string start_key, int Random_bit, int FuncLevel, int searchMode,
	...
	//if (FunctionLevel >= 3){
		bool start_seed_fl = true;//start_seed_fl = true;
	//}
	//if (FunctionLevel >= 4){
		keys_seed_fl = true;
	//}
	...
	*/
	...
	#ifdef WIN64
	RAND_screen();
	printf("[i] OpenSSL RAND_screen() OK\n");
	#endif
	...
	printf("[i] BITS: %d Option: -bits 67 (use 20-256)\n", Random_bits);
	printf("\n");
	// Start bit 66 - 32 BTC Puzzle
	//Int startBIT66;
	//startBIT66.SetBase16("38000000000000000");// ? start
	//
	Key_cnt = 0;
...
bool VanitySearch::checkPrivKey(string addr, Int &key, int32_t incr, int endomorphism, bool mode) {
  ...
  */
  ...
  #ifdef WIN64 
  printf("\nPlaying wav file buzzer.wav\n");
  bool ps1 = PlaySound(TEXT("buzzer.wav"), NULL, SND_FILENAME);
  //bool ps1 = PlaySound(TEXT("SystemStart"), NULL, SND_ALIAS);
  #else 
  ...
  while (ok && !endOfSearch) {
    ...
    if (ok) {
      for (int i = 0; i < nbThread; i++) {
		...
		keys[i].Add((uint64_t)(STEP_SIZE * NB_SPIN));// !!!
      }
      //counters[thId] += 6ULL * STEP_SIZE * nbThread; // Point +  endo1 + endo2 + symetrics
	  //counters[thId] += STEP_SIZE * nbThread;
	  //counters[thId] += 2ULL * (uint64_t)STEP_SIZE * (uint64_t)NB_SPIN * (uint64_t)nbThread;
	  counters[thId] += (uint64_t)STEP_SIZE * (uint64_t)NB_SPIN * (uint64_t)nbThread;
    }
...
void VanitySearch::GetSeedBIP39(int thId, bool fl, std::string &s_seed, bool word_flag) {
	...
	*/
	...
	//unsigned char wn[max_size];
	// old code - mod 010
	
	for (i = 0; i < max_size; i++) {// 16
		if (word_flag) {
			//rand1 = rndl() % 2048;// original size 2048 word of list 
			//rand1 = (unsigned long)rndll() % 2048;// original size 2048 word of list 
			rand1 = (unsigned long)rndll() & 2047;
			//rand1 = rndl() & 0x000000FFUL;// !!! low entropy
		} else {
			//rand1 = rndl() & 0x000000FFUL;// for length 32 or 256 bits
			//rand1 = (unsigned long)rndll() & 0x000000FFUL;// for length 32 or 256 bits
			rand1 = (unsigned long)rndll();
		}
		data_rndl[i] = (uint32_t)rand1;
		//printf("\n data_rndl[%d]: %lu ranl(): %lX\n", i, data_rndl[i], rand1);
		
		//wn[i] = (unsigned char)rand1 & 0xFF;
	}
	/*
	// mod 011
	// Run 8 rndl() !!!
	...
	*/ 
	...
	if (word_flag) {
		...
		for (i = 0; i < nb_word; i++) {// 1-24
			uint32_t ind = data_rndl[i] & 2047;//uint32_t ind = data_rndl[i];
			tmp = word_list[ind];// #include bip39_english.h std::string word_list[2048] 
			tmp_out.append(tmp);
			if (i < (nb_word - 1)) {
...
void VanitySearch::drvKey(std::string &InData, std::string &InKey, std::string &outMaster, std::string &outChain, bool master_key_fl, uint32_t key_ind, uint8_t depth_ind, std::string &extended_key, Int &outIL, Int &outIR, std::string &pubkey) {
	...
	if (Hardened_Child_flag) {
		...
		//in_data = "00" + outMaster + index_str;
	}
...
void VanitySearch::getKeysFromRandomSeedPRNG(int thId, int nbitU, bool word_fl, bool master, int nbThread, Int *keys) {
	...
	Hardened_Child_flag = true;
	Extended_flag = true;//Extended_flag = false;
	...
	//printf("\n[i] Nb Child: %d Key dIR: %s", n, dIR.GetBase16().c_str());	
		...
		/* // Block Explorers - Use path m/44'/0'/0'. 
		// Path - m/44'/0'/0'
		// Purpose 44
		// Coin 0
		// Account 0
		for (n = 0; n < 2; n++) {
			in_key = masterChainKey;
			Hardened_Child_flag = true;
			//Extended_flag = false;
			keyIndex = 0;
			depth_index_u8 = 2 + n;
		...
		*/
		...
		// Check output
		if (verbose_fl >= 4) {
			Point po = secp->ComputePublicKey(&dIL);
			// Address
			string Output_pub_key = secp->GetPublicKeyHex(true, po);
			string Output_addr = secp->GetAddress(0, 1, po);
			//printf("\n[i] Path - m/0'/0'/0' ");
			printf("\n[i] Path - m/0'/0'/66' ");
			printf("\n[i] Output Pub key: %s ", Output_pub_key.c_str());
			printf("\n[i] Output address: %s \n", Output_addr.c_str());
		}
		...
		if (Key_cnt > 1) {
			...
			k.SetInt32(0);
			k.Set(&PL.x);
			//printf("\n[i] Output %d PointX : %s ", Key_cnt, PL.x.GetBase16().c_str());
		} else {
			Point PL0 = secp->ComputePublicKey(&dIL);
			k.SetInt32(0);
			k.Set(&PL0.x);
			//printf("\n[i] Output %d PointX0: %s ", Key_cnt, PL0.x.GetBase16().c_str());
		}
		
		//uint32_t rb = 256 - (uint32_t)nbitU;
		//k.ShiftR(rb);// 256 - 66
		pKey.Set(&k);
		//printf("\n[i] Output %d PointX to pKey: %s ", Key_cnt, pKey.GetBase16().c_str());
		...
		*/
		...
		if (nbitU == (int)TARGET_KEY_BITS) {
			...
			if (len > nbitU) {//goto newSeed;
				//sKey.SetByte(8, (unsigned char)TARGET_KEY_HIGH_BYTE_L2);// 0x3
				sKey.SetByte(8, (unsigned char)TARGET_KEY_HIGH_BYTE_L1);// 0x2
				
			}
			//sKey.SetByte(7, (unsigned char)0x00);// 0x00
			
		}
		
		//if (len1 < 66) { goto newSeed; }// !!!!!!!!!!!!!!!!!

...
void VanitySearch::Search(int nbThread,std::vector<int> gpuId,std::vector<int> gridSize) {
  ...
  while (isAlive(params)) {
    ...
    while (isAlive(params) && delay>0) {
      Timer::SleepMillis(50);//Timer::SleepMillis(500);
      delay -= 500;
    }
    ...
    if (rekey > 0) {
      if ((count - lastRekey) > (100000 * rekey)) {// if ((count - lastRekey) > (1000000 * rekey)) {
        // Rekey request
        rekeyRequest(params);
        lastRekey = count;
      }
...
void VanitySearch::GetSeedBIP39(int thId, bool fl, std::string &s_seed, bool word_flag) {
	...
	if (!Brainwallet_fl && FunctionLevel == 1) {
		...
		// SET SALT !!
		salt = "Bitcoin seed";
		//salt = "VanitySearch";// Set any salt !!
		//
		// pbkdf2_hmac_sha512()
...
void VanitySearch::drvKey(std::string InData, std::string InKey, std::string &outMaster, std::string &outChain, uint64_t nb_iter, std::string &extended_key, Int &outIL, Int &outIR) {
	
	string seed = InData;// seed 64 bytes
	string salt = InKey;// salt
	//uint64_t iter = nb_iter;
	
	// !!! Use wrapper pbkdf2_hmac_sha512()
	//uint8_t hseed[64];// Output data
	
	//pbkdf2_hmac_sha512(hseed, (size_t)64, (const uint8_t *)seed.c_str(), (size_t)seed.length(), 
	//(const uint8_t *)salt.c_str(), (size_t)salt.length(), 
	//iter);
	
	//printf("\n drvKey () seed: %s", seed.c_str());
	//printf("\n drvKey () salt: %s\n", salt.c_str());
	//printf("\n drvKey () iter: %llu\n", iter);
	
	// HMAC sha512
	// Output data hmac_sha512() unsigned char hseed
	unsigned char hseed[64];
	hmac_sha512((unsigned char *)salt.c_str(), (int)salt.length(), (unsigned char *)seed.c_str(), (int)seed.length(), hseed);
	
	// Reverse bytes
	//uint8_t hseed_r[64];// if use this pbkdf2
	...
	// debug printf
	//printf("\n[i] masterKey IL: %s ", IL.GetBase16().c_str());
	//printf("\n[i] chainKey  IR: %s ", IR.GetBase16().c_str());
	...
	masterSecretKey = IL.GetBase16().c_str();// 32 bytes
	masterChainKey = IR.GetBase16().c_str();// The chain code is just an extra 32 bytes that we couple with the private key to create what we call an extended key. 
	
	//We use these 64 bytes to create our master extended private key.
	extended_private_key.append(masterSecretKey);
	extended_private_key.append(masterChainKey);
	
	// Output 
	outMaster = masterSecretKey;
	outChain = masterChainKey;
	extended_key = extended_private_key;
	...
	key = prepend + public_key;
	// Get check sum
	string in_checksum = version + depth + parent_fingerprint + child_index + chain_code + key;	
	string checksum = "";// First 4 bytes
	
	// Double sha256() of in_checksum
...
void VanitySearch::getKeysFromRandomSeedPRNG(int thId, int nbitU, bool word_fl, bool master, int nbThread, Int *keys) {
	...
	if (Brainwallet_fl) {
		...
		uint32_t j = 0;		
		for(j = 0; j < nb; j++) 
			sKey.bits[j] = bKey.bits[j];
		sKey.bits[j] = bKey.bits[j] & mask;
		// Set output
		keys[0].Set(&sKey);
		bKey.SetInt32(0);// pause ?
		return;
	...
	// HMAC-SHA512
	drvKey(seed, salt, masterSecretKey, masterChainKey, iter_n, extended_key, dIL, dIR);
	
	// test output data
	// e824627dd38648b6aae958788c954ac0cfb8f400235cc80cb984565c7b186499 
	// b8b6448669c39982194af0013da78c337497f372d128e9205be78ea105788b2b 
	// url:
	...
	//printf("\n-> Master extended_key: %s", extended_key.c_str());
	
	// Derivation Key
	//volatile int nbChild = 1;	
	//string in_msg = "";
	//string parent_public_key = "";
	//string parent_public_key_index = "";
	...
	// |- m/2
	// ...	
	//string parent_chain_code  = "463223aac10fb13f291a1bc76bc26003d98da661cb76df61e750c139826dea8b";
	//string parent_private_key = "f79bb0d317b310b261a55a8ab393b4c8a1aba6fa4d08aef379caba502d5d67f9";
	//string parent_public_key = "0252c616d91a2488c1fd1f0f172e98f7d1f6e51f8f389b2f8d632a8b490d5f6da9";
	//int ind = 0;// child index number // 32-bit unsigned	
	
	//in_msg = parent_public_key + to_string(ind);// for check	
	//printf("\n[i] ! in_msg: %s ", in_msg.c_str());
	
	//string in_key = masterChainKey;
	//string in_key = parent_chain_code;// for check
	
	//Int child_key;// m/0
	//child_key.SetInt32(0);
	
	//child_key.SetBase16("f79bb0d317b310b261a55a8ab393b4c8a1aba6fa4d08aef379caba502d5d67f9");// for check	
	//child_key.Set(&dIL);// set masterSecretKey
	
	//for (i = 0; i < nbThread; i++) {
	//for (i = 0; i < nbChild; i++) {
		
		// HMAC-SHA512
		//drvKey(in_msg, in_key, masterSecretKey, masterChainKey, iter_n, extended_key, dIL, dIR);
		
		//child_key.ModAddK1order(&dIL);// add child masterSecretKey
		
		//printf("\n[i] ! child_key %d: %s ", i, child_key.GetBase16().c_str());
		
		// debug printf
		//printf("\n[i] Nb Child: %d masterSecretKey: %s ", i, masterSecretKey.c_str());
		//printf("\n[i] Nb Child: %d masterChainKey:  %s ", i, masterChainKey.c_str());		
		//printf("\n[i] Nb Child: %d Key dIL: %s ", i, dIL.GetBase16().c_str());
		//printf("\n[i] Nb Child: %d Key dIR: %s ", i, dIR.GetBase16().c_str());
	//}	
		...
		// Switch IL or IR
		if (1) {//if (master) {
			pKey.Set(&dIL);// IL as master secret key
		} else {
			pKey.Set(&dIR);// IR as master chain code
		}

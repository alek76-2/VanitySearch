...
VanitySearch::VanitySearch(Secp256K1 *secp, vector<std::string> &inputPrefixes, string seed, string start_key, int Random_bit, int FuncLevel, int searchMode,
  ...
  if (Seed.length() > 0) {
	  ...
	  startKey.Set(&Sk[0]);
	  printf("[i] startKey.Set() from mod_006 Seed: %s \n", Seed.c_str());
	  //
  }
  delete [] Sk;
  //
...
void VanitySearch::getCPUStartingKey(int thId, Int& key, Point& startP) {
  ...
  printf("\n[i] Generate Keys HMAC-SHA512 %d Rounds and sha256() From Random SEED ", (int)hmac_sha512_nb_round); 
  ...
  if (rekey > 0) {
    NewRandom:	
	//
	getKeysFromRandomSeed(BitL, BitU, true, 1, SeedKeys);
	Timer::SleepMillis(1);
	//
	//key.Rand(Random_bits);// bit 66
	key.Set(&SeedKeys[0]);// from Seed
...
void VanitySearch::getGPUStartingKeys(int thId, int groupSize, int nbThread, Int *keys, Point *p) {
  ...
  // Keys from Seed
  Int *SeedKeys = new Int[1];//Int *SeedKeys = new Int[nbThread];
  int BitL = Random_bits - 1;
  int BitU = Random_bits;
  
  printf("\n[i] Generate Keys HMAC-SHA512 %d Rounds and sha256() From Random SEED ", (int)hmac_sha512_nb_round); 
  
  //getKeysFromRandomSeed(BitL, BitU, true, nbThread, SeedKeys);
  ...
  for (int i = 0; i < nbThread; i++) {
	...
	if (rekey > 0) {
      NewRandom:
	  keys[i].SetInt32(0);// CLR !!!
	  //keys[i].Rand(Random_bits);// BIT 66
	  // from Seed
	  getKeysFromRandomSeed(BitL, BitU, true, 1, SeedKeys);
	  keys[i].Set(&SeedKeys[0]);//keys[i].Set(&SeedKeys[i]);// from Seed
	  //
	  //key2.SetBase16("20000000000000000");// min value bit 66
	  //key3.SetBase16("3ffffffffffffffff");// max value bit 66
	  //
	  ...
	  //
	  if (i < 10) printf("\n[i] Generate nb: %d Keys: %s HMAC-SHA512 Random SEED ", i, SeedKeys[0].GetBase16().c_str());
	  //
    } else {
      //
	  keys[i].SetInt32(0);// CLR !!!
	  ...
	  //
	  if (i < 10 || i > nbThread - 10) printf("[i] SeedKeys %d: %s \n", i, SeedKeys[i].GetBase16().c_str());
	  //
    }
    //Int k(keys + i);
    ...
    k.Add((uint64_t)(groupSize / 2));
	// Check 
	printf("Check Send in GPU Keys %d: %s startPubKeySpecified: %d \n", i, k.GetBase16().c_str(), startPubKeySpecified);
	//
    p[i] = secp->ComputePublicKey(&k);
    if (startPubKeySpecified)
      p[i] = secp->AddDirect(p[i], startPubKey);
...
void VanitySearch::FindKeyGPU(TH_PARAM *ph) {
  ...
  //getGPUStartingKeys(thId, g.GetGroupSize(), nbThread, keys, p); Why call the function 2 times???
...
void VanitySearch::getKeysFromRandomSeed(int nbitL, int nbitU, bool master, int nbThread, Int *keys) {
	...
	for (int i = 0; i < nbThread; i++) {
		...
		*/
		...
		sha256(hseed, 64, (unsigned char *)pKey.bits64);// Used this function !!!
		...
		//if (nbitU == 66) {
			...
			//sKey.SetByte(8, (unsigned char)KEY66_HIGH_BYTE);// The high byte is 0x03 or 0x2 
			//printf("\n[i] Nb: %d sKey: %s ", i, sKey.GetBase16().c_str());			
		//}
